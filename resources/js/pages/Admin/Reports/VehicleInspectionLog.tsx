...
function FilterBar({ drivers, vehicles, filters }: { drivers: any[]; vehicles: any[]; filters: any }) {
    ...
    const exportUrl = () => {
        ...
        return route('admin.reports.vehicle-log') + '?' + params.toString();
    };
...
export default function VehicleInspectionLog({ deliveries, drivers, vehicles, filters }: Props) {
    return (
        <AdminLayout breadcrumbs={[
            ...
            <Head title="Daily Vehicle Cleaning & Inspection Log" />
            <div className="flex flex-1 flex-col gap-6 p-4 lg:p-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Pre-trip vehicle inspection checklist records — cleanliness, HVAC, cargo separation, contamination status, and driver info.
                    </p>
                </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="border-b bg-gray-50 text-xs font-medium uppercase text-gray-500 dark:border-gray-700 dark:bg-gray-800/50 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-center">Pre-Trip Complete</th>
                                    <th className="px-4 py-3 text-center">Cargo Disinfected</th>
                                    <th className="px-4 py-3 text-center">Separation Verified</th>
                                    <th className="px-4 py-3 text-center">Spill Kit Stocked</th>
                                    <th className="px-4 py-3 text-center">PPE Stocked</th>
                                    <th className="px-4 py-3 text-center">No Contamination</th>
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                                {deliveries.length === 0 ? (
                                    <tr>
                                        <td colSpan={10} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400 font-medium">
                                    ...
                                    return (
                                        <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 font-mono text-xs font-semibold text-gray-700 dark:text-gray-300">
                                                {d.vehicle?.vehicle_number || <span className="text-gray-400 italic">N/A</span>}
                                            </td>
